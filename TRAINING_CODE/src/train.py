...
from datasets import PairedImgDataset, SingleImgDataset
...
val_dataset = SingleImgDataset(data_source=opt.data_source, mode='val')
...
model = NAFNet(img_channel=3, width=opt.width, middle_blk_num=opt.middle_blk_num,
               enc_blk_nums=opt.enc_blk_nums, dec_blk_nums=opt.dec_blk_nums).cuda()

model_val = NAFNetLocal(img_channel=3, width=opt.width, middle_blk_num=opt.middle_blk_num,
                        enc_blk_nums=opt.enc_blk_nums, dec_blk_nums=opt.dec_blk_nums,
                        train_size=(1, 3, opt.crop, opt.crop)).cuda()
...
def main():
    ...
    for epoch in range(start_epoch, opt.n_epochs + 1):
        ...
        train_dataset.dataset_refresh()
        val_dataset.dataset_refresh()
...
def train(epoch, optimal):
    ...
    for i, (imgs, gts) in enumerate(train_dataloader):
        ...
        if i == 0:
            save_image(torch.cat((F.interpolate(imgs, scale_factor=4, mode='area'),preds.detach(),gts),0), train_images_dir + '/epoch_{:0>4}_iter_{:0>4}.png'.format(epoch, i+1), nrow=opt.train_bs, normalize=True, scale_each=True)
...
def val(epoch, optimal):
    ...
    for i, (imgs, name) in enumerate(val_dataloader):
        imgs, name = imgs.cuda(), name
        ...
        if i == 0:
            if epoch == opt.val_gap:
                save_image(F.interpolate(imgs, scale_factor=4, mode='area'), val_images_dir + '/epoch_{:0>4}_iter_{:0>4}_img.png'.format(epoch, i+1), nrow=opt.val_bs, normalize=True, scale_each=True)
