...
from team01_AsyNAF_1 import RLFN_Prune, RLFN_PruneLocal
...
test_dataset = SingleImgDataset(data_source=opt.data_source, mode='test')
...
if opt.train_crop is None:
    ...
    #model = RLFN_Prune().cuda()
else:
    model = NAFNetLocal(img_channel=3, width=opt.width, middle_blk_num=opt.middle_blk_num,
                        enc_blk_nums=opt.enc_blk_nums, dec_blk_nums=opt.dec_blk_nums,
                        train_size=(1, 3, opt.train_crop, opt.train_crop)).cuda()
    #model = RLFN_PruneLocal().cuda()
...
def main():
    ...
    with open(txt_path, 'w') as f:
        for i, (img, name) in enumerate(test_dataloader):
            ...
            print('Iteration[' + str(i+1) + '/' + str(len(test_dataset)) + ']' + '  Processing image... ' + name[0] + '  Time ' + str(times))
            f.write('Iteration[' + str(i+1) + '/' + str(len(test_dataset)) + ']' + '  Processing image... ' + name[0] + ' Time: ' + str(times) + '\n')
